...
inline
void
qo_dec64_to_fixed17_str_avx512ifma(
    uint64_t x ,
    char* buffer
) {
    uint64_t n_15_08  = x / 100000000;
    uint64_t n_07_00  = x % 100000000;
    ...
    __m512i permb_const	= _mm512_castsi128_si512(
        _mm_set_epi8(0x78, 0x70, 0x68, 0x60, 0x58,
                    0x50, 0x48, 0x40, 0x38, 0x30, 0x28, 
                    0x20, 0x18, 0x10, 0x08, 0x00
        )
    );
    ...
    __m512i highbits_h	= _mm512_madd52hi_epu64(ascii_zero, zmmTen, lowbits_h);
    __m512i highbits_l	= _mm512_madd52hi_epu64(ascii_zero, zmmTen, lowbits_l);
...
inline
void
qo_bin64_to_fixed64_str_avx512(
    uint64_t value ,
    char * buffer
) {
    ...
    uint64_t swapped_value = __builtin_bswap64(value);
...
inline
void
qo_hex64_to_fixed17_str_avx512(
    uint64_t value , 
    char * buffer
) {
    ...
    __mmask64 is_digit = _mm512_cmplt_epi8_mask(nibbles, _mm512_set1_epi8(10));
    __m512i digit_0_9 = _mm512_set1_epi8('0');
    __m512i digit_a_f = _mm512_set1_epi8('a' - 10);
    __m512i digit_offset = _mm512_mask_blend_epi8(is_digit, digit_a_f, digit_0_9);
    __m512i hex_chars = _mm512_add_epi8(nibbles, digit_offset);
    
    // Store the result (16 bytes)
    _mm_storeu_si128((__m128i*)buffer, _mm512_extracti32x4_epi32(hex_chars, 0));
...
inline
void
qo_hex64_to_fixed17_str_avx512vl(
    uint64_t x ,
    char* buffer
) {
    alignas(16) static const uint8_t hex_table[16] = {
    ...
    __m128i result = _mm_shuffle_epi8(
        _mm_load_si128((const __m128i*)hex_table),
        high_low
    );
...
inline
bool // Return: Overflow?
qo_decstr_to_u64_avx512(
    const char* begin ,
    const char* end ,
    uint64_t * p_value
) {
    size_t digit_count = size_t(end - begin);
  ...
  uint32_t mask = uint32_t(0xFFFFFFFF) << (begin - end + 32);
